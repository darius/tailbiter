...
class Instruction(Assembly):
    # The instruction encoding has changed: now every instruction takes two bytes, 
    # including an argument, which is 0 if the instruction doesn't use it.
    length = 2
...
class CodeGen(ast.NodeVisitor):
    ...
    def visit_Dict(self, t):
        return (concat([self(k) + self(v)
                        for k, v in zip(t.keys, t.values)])
                + op.BUILD_MAP(len(t.keys)))
